...
<?php
  ...
  $page = $_GET['page'];
	...
	if(isset($_GET["s"])){
		...
		$query = "select * from tbl_news_h where curdate()<=end_date;";
	}
	//echo $query;
	$rows = 0;
	$select_post = mysqli_query($connection,"set Names utf8;");
    ...
    $count = ceil($count /3);
?>
<div id="all" style="float: left; display:table; width: 60%; ">
<?php		
		

	 if(isset($_GET["s"])){
		$query = "select * from tbl_news where curdate()<=end_date and categoryID=(select newscat_id from tbl_news_category where newscat_detail=".$_GET["s"].") order by date_published desc Limit ".$page1.",3;";
		//echo $query;
	} else {
		$query = "select * from tbl_news_h where curdate()<=end_date order by date_published desc  Limit ".$page1.",3;";
	}	 
	$select_post = mysqli_query($connection, $query);
	...
	$query1 = "Select name from tbl_tag	where id in (Select tagID from tbl_news_tag where newsID=".$post_id.") group by name;";
		...
		if(isset($_COOKIE["User"]) && isset($_POST["like"])){
				$q = "Select * from tbl_Liked_news_h where userid=".$_COOKIE["User"]. " and newid=".$_POST["n"].";";
				//echo $q;
			...
			if($number){
				$q="Delete from tbl_Liked_news_h where userid=".$_COOKIE["User"]." and newid=".$_POST["n"].";";
				//echo $q;
				$r=mysqli_query($connection,$q);
				//echo mysqli_error($connection);
				$like="Like";
			}else{
				$q="Insert into tbl_liked_news_h(userid,newid) values(".$_COOKIE["User"].",".$_POST["n"].");";
				//echo $q;
				$r=mysqli_query($connection,$q);
				//echo mysqli_error($connection);
				$like="Unlike";
                ...
                while ($row = mysqli_fetch_assoc($select_post)) {
					...
					$q = "Select newscat_name from tbl_news_category where newscat_id in (select categoryid from tbl_news_h where newss_id=".$post_id.");";
					//echo $q;
					$r = mysqli_query($connection,$q);
					//echo mysqli_error($connection);
					...
					if($sign){
						$q = "Select * from tbl_Liked_news_h where userid=".$_COOKIE["User"]. " and newid=".$post_id.";";
				//echo $q;
			$r = mysqli_query($connection,$q);
			//echo mysqli_error($connection);
			$number = mysqli_num_rows($r);
			...
			$q1 = "Select count(*) from tbl_Liked_news_h where newid=".$post_id.";";
			//echo $q1;
			...
			$q1 = "Select count(*) from tbl_comments_h where comment_news_id=".$post_id.";";
			//echo $q1;
			$r1 = mysqli_query($connection,$q1);
			//echo mysqli_error($connection);
			$row1 = mysqli_fetch_array($r1);
...
<div class="news" id='cb<?php echo $post_id; ?>'>
	<div class="container">
		...
		<a href="CommentHindi.php?news=<?php echo $post_id; ?>"><img name="image" id="image" style="box-shadow: 3px 3px 3px gray;" src="../images/news/<?php echo $post_image; ?>" alt=""></a>

		<div class='title'> <a href="CommentHindi.php?news=<?php echo $post_id; ?>"><?php echo $post_title; ?></a> </div>
		<div class='desc' id='ab<?php echo $post_id; ?>'><?php echo $post_desc; ?></div>
		<div class='fulldesc' id='eb<?php echo $post_id; ?>' style='clear: left; display:none;'>
		<?php echo $post_detail; ?></div>
	...
	<div class="buttons" style='clear: left; width: 95%;'>
		...
		<a style="text-decoration: none; color: black;" href="./CommentHindi.php?news=<?php echo $post_id; ?>" > <div style="float:right;  "><span id="cmnd<?php echo $post_id; ?>" style='clear:left; float: right;'> See <?php if($t_cmnt>0) echo $t_cmnt; ?> Comments  </span> </div> </a>
		<div style="float:right;  ">
			<form style="display:initial;" name="myform" onsubmit="return check();" action="<?php if($sign){ echo htmlspecialchars($self1); }?> " method="post">
