...
<?php
  ...
  $page = $_GET['page'];
    ...
    if ($page == "" || $page == 1) {
      ...
      //global $page1;
      $page1 = ($page * $rowsofpage) - $rowsofpage; 
    }
...
<?php		
	 ...
	 if(isset($_GET["s"])){
		$query = "select * from tbl_news where categoryID=(select newscat_id from tbl_news_category where newscat_detail=".$_GET["s"].") order by date_published desc Limit ".$page1.",".$rowsofpage.";";
		//echo $query;
	} else {
		$query = "select * from tbl_news order by date_published desc Limit ".$page1.",".$rowsofpage.";";
	}
...
<div class="news" id='cb<?php echo $post_id; ?>'>
	...
	<div class="buttons" style='clear: left; width: 95%;'>
		...
		<div style="float:right;  ">
			...
			<span id="lk<?php echo $post_id; ?>"  style=' clear:left; float: right; padding-right: 30px;'> <input type="submit" name="like" onsubmit="" value="&hearts; <?php echo $like; ?>" > <span><?php echo $total." Liked"; ?>  </span>  </input></span>
				</form>
		</div>
	</div>
