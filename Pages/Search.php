...
<?php
  ...
  $page = $_GET['page'];
	...
	$srchstring1 = strtoupper($_GET["stxt"]);
	$query = "select * from tbl_news where curdate()<=end_date and (upper(tittle) in \"".$srchstring1."\" or upper(details) in \"".$srchstring1."\" ) ;";
...
<div align="center" style="box-shadow: 3px 3px 3px gray; border-radius: 20px; border: 1px solid lightblue; /*border-bottom: 3px solid pink;*/ margin-bottom: 30px; width: 300px; margin:auto; margin-top: 100px;"> <h1 style="color:blue; text-shadow: 1px 1px 1px gray;"> Search Result </h1> </div>
<br />
<br />
<?php		
		

	$query = "select * from tbl_news where curdate()<=end_date and (tittle like \"%".$_GET["stxt"]."%\" or details like \"%".$_GET["stxt"]."%\" ) Limit ".$page1.",3 ;";	 
	$select_post = mysqli_query($connection, $query);
...
if($rows==0){
	...
	echo "<div style='position: relative; left: 30px; width: 60%; font-size: 30px; font-family: \"Courier New\";'> No result found for <i>\"".$_GET["stxt"]."\"<i>....	</div>";
}
