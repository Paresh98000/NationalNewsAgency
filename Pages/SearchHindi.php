...
<?php
  ...
  $page = $_GET['page'];
	...
	$query = "select * from tbl_news_h where curdate()<=end_date and (tittle like \"%".$_GET["stxt"]."%\" or details like \"%".$_GET["stxt"]."%\" ) ;";
...
<div align="center" style="box-shadow: 3px 3px 3px gray; border-radius: 20px; border: 1px solid lightblue; /*border-bottom: 3px solid pink;*/ margin-bottom: 30px; width: 300px; margin:auto; margin-top: 100px;"> <h1 style="color:blue; text-shadow: 1px 1px 1px gray;"> तलाश का परीणाम </h1> </div>
<br />
<br />
<?php		
		

	$query = "select * from tbl_news_h where curdate()<=end_date and (tittle like \"%".$_GET["stxt"]."%\" or details like \"%".$_GET["stxt"]."%\" ) Limit ".$page1.",3 ;";	 
	$select_post = mysqli_query($connection, $query);
...
<div class="news" id='cb<?php echo $post_id; ?>'>
	...
	<div class="buttons" style='clear: left; width: 95%;'>
		...
		<div style="float:right;  ">
			<form style="display:initial;" name="myform" method="POST" onsubmit="return check();" action="<?php if($sign){ echo htmlspecialchars($_SERVER['PHP_SELF'])."?stxt=".$srchtext."&&page=".$page	; }?> " >
...
<div class="pager" style="text-align:center; align: center; padding: 10px; heigh: 100px;"> 
                  ...
                  for ($i=1; $i <= $count; $i++) {
                    $self1="";
						...
						$self1=$_SERVER["PHP_SELF"]."?stxt=".$srchtext."&&";
...
if($rows==0){
	...
	echo "<div style='position: relative; left: 30px; width: 60%; font-size: 30px; font-family: \"Courier New\";'> <i>\"".$_GET["stxt"]."\"<i> से जुडा कोइ संदेश या समाचार नही मीला....	</div>";
}
