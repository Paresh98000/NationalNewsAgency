...
mysqli_query($connection,'SET collation_connection=utf8_general_ci');
	...
	if(isset($_GET["news"])){
		$query = "select * from tbl_news_h where newss_id=".$_GET["news"].";";
		//echo $query;
	...
	if(isset($_POST["submit"])&&$sign){
		$q = "Insert into tbl_comments_h (comment_news_id,comment_author,comment_content,comment_date) values (".$_GET["news"].",".$_COOKIE["User"].",\"".$_POST["comnt"]."\",sysdate());";
		//echo $q;
		mysqli_query($connection,'set names utf8;');
		$r = mysqli_query($connection,$q);
		echo mysqli_error($connection);
...
<section style="position: relative; top: 110px; float: left; left: 70px;  width: 60%; margin-right: 100px; min-width: 800px; margin-bottom: 150px;">
	...
	<div class="news" style="border-radius: 8px; border: 1px solid lightgray; padding: 10px; heigh: 480px;overflow: auto;">
		...
		<div id="desc" style="font-size: 20px; padding: 10px 5px;">
			...
			<p><?php echo $post_detl; ?></p>
		</div>	
	</div>
	
	<div class='comment' style='padding: 10px; border: 1px solid Gray; position: relative; top: 10px; border-radius: 10px;  max-height: 450px; overflow: auto;'>
				...
				<?php
					if(isset($_GET["news"])){
						$q = "Select * from tbl_comments_h where comment_news_id=".$_GET["news"]." order by comment_date desc ;";
						$r = mysqli_query($connection,'set names utf8;');
						$r = mysqli_query($connection,$q);
						//echo mysqli_error($connection);
